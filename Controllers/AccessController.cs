...
namespace ePortal.Web.Controllers
{
    ...
    public class AccessController : Controller
    {
        ...
        public AccessController(ICommandBus commandBus, IEntityRepository<Employee_Full> employeeRepository, IEntityRepository<Scheduling> schedulingRepository, IEntityRepository<AccessFormMode> AccessFormModelRepository, IEntityRepository<Absence> absenceRepository, IEntityRepository<Overtime> overtimeRepository, IEntityRepository<AbnormalAnalysis> analysisRepository, IEntityRepository<AccessRecord> AccessRecordRepository, IEntityRepository<Bide> BideRepository, IEntityRepository<MailingList> mailingListRepository, IUnitOfWork UnitOfWork)
        {
        ...
        {  //If date is not passed, take current month's first and last dte 
            ...
            else
            {
                ...
                string TotalResult = "0";
                var entityModel = AccessFormModelRepository.SqlQuery<ePortal.Web.ViewModels.AccessFormModel>("sp_Accesswith", out TotalResult, new SqlParameter[]
                {
                ...
                IEnumerate entityResult = new Enumerate<AccessFormModel>(entityModel, PageIndex, PageSize, Convert.ToInt32(TotalResult));
                ...
                {
                    return PartialView("Access/Access", entityResult);
                } 

                 return View("Access/Index", entityResult);
        ...
        [HandleErrors]
        public FileResult attendancexls(string StartDate, string EndDate, string empno)
        {
            Response.Clear();
            Response.AppendHeader("content-disposition", "attachment;filename=\"" + System.Web.HttpUtility.UrlEncode("Access", System.Text.Encoding.UTF8) + ".xls\"");
            ...
            using (System.IO.StringWriter writer = new System.IO.StringWriter())
            {
                ...
                Access.AccessModel = AccessFormModelRepository.SqlQuery<AccessFormModel>("sp_Accesswith_xls {0},{1},{2}", new object[] { StartDate, EndDate, empno }).ToList();
                IEnumerate entityResult = new Enumerate<AccessFormModel>(Access.AccessModel);
                
                viewName = "Access/_Accessxls";                      

                IView view = ViewEngines.Engines.FindPartialView(ControllerContext, viewName).View;
                var Partia = PartialView(viewName, entityResult);
                ViewContext viewContext = new ViewContext(ControllerContext, view, Partia.ViewData, Partia.TempData, writer);
                viewContext.View.Render(viewContext, writer);
                htmlcol.InnerHtml = writer.ToString();
                var file = ToExcel(htmlcol, "~/temp/Access");
