...
{
    /*
     * summary:AttendanceController廠區門禁模塊Controller
     * 
     */
    [CompressResponse]
    public class OthersController : Controller
    {
        ...
        public ActionResult Audit(int? ResultId, DateTime? StartDate, DateTime? EndDate, string empno, string Report_Type)
        {  //If date is not passed, take current month's first and last dte 
            ...
            empno = empno.TrimEnd(',').TrimEnd(',');


            //if(empno!="")
            //{
            //    //對多工號處理
            //    string[] employes = empno.Split(',');

            //    string empno_new = "";
            //    foreach (string item in employes)
            //    {
            //        empno_new = empno_new + "'" + item + "',";
            //    }

            //    empno = empno_new.TrimEnd(',') ;
            //} 

            var entityModel = OthersFormModelRepository.SqlQuery<ePortal.Web.ViewModels.OthersFormModel>("sp_Otherwith", out TotalResult, new SqlParameter[]
                {
                    ...
                    new System.Data.SqlClient.SqlParameter{DbType=System.Data.DbType.String,Value=empno,ParameterName="empno",Size=1000},
                    new System.Data.SqlClient.SqlParameter{DbType=System.Data.DbType.String,Value=Report_Type,ParameterName="Report_Type",Size=50}             
                }).ToList();

            IEnumerate entityResult = new Enumerate<OthersFormModel>(entityModel, PageIndex, PageSize, Convert.ToInt32(TotalResult));
            ...
            {
                return PartialView("Others/Others", entityResult);
            }
            return View("Others/Index", entityResult);           
        ...
        /// <returns></returns>
        [HandleErrors]
        public ActionResult Analyse(string StartDate, string EndDate, string empno, string Report_Type)
        {           
            ...
            switch (Report_Type)
            {
                case "1":
                    //顯示廠區大門記錄
                    var Access = new AnalyseForm();
                    Access.Access = AccessFormModelRepository.SqlQuery<AccessFormModel>("sp_Accesswith_Record {0},{1},{2},{3}", new object[] { StartDate, EndDate, empno, Report_Type }).ToList();;
                                             
                    return PartialView("Others/_Access", Access);
                 

                case "2":
                    //顯示車間門禁記錄
                    var Plant = new AnalyseForm();
                    Plant.Access = AccessFormModelRepository.SqlQuery<AccessFormModel>("sp_Accesswith_Record {0},{1},{2},{3}", new object[] { StartDate, EndDate, empno, Report_Type }).ToList();;
                             
                    return PartialView("Others/_Plant", Plant);

                   
                case "3":
                    //顯示考勤記錄
                    var Atendence = new AnalyseForm();
                    Atendence.Access = AccessFormModelRepository.SqlQuery<AccessFormModel>("sp_Accesswith_Record {0},{1},{2},{3}", new object[] { StartDate, EndDate, empno, Report_Type }).ToList();;
                    return PartialView("Others/_Atendence", Atendence);

                case "4":

                    //顯示HCP記錄
                    var HCP = new AnalyseForm();
                    HCP.HCP = HCPRepository.SqlQuery<HCPFormModel>("sp_Accesswith_Record {0},{1},{2},{3}", new object[] { StartDate, EndDate, empno, Report_Type }).ToList();;
                    return PartialView("Others/_HCP", HCP);      

                default:
                  return PartialView("Others/Index");;      
        ...
        [HandleErrors]
        public FileResult attendancexls(string StartDate, string EndDate, string empno, string Report_Type)
        { 
            Response.Clear();
            Response.AppendHeader("content-disposition", "attachment;filename=\"" + System.Web.HttpUtility.UrlEncode("Others", System.Text.Encoding.UTF8) + ".xls\"");
              ...
              switch (Report_Type)
            {
                ...
                case "3":
                    ...
                    Access.Access = AccessFormModelRepository.SqlQuery<AccessFormModel>("sp_Accesswith_Record {0},{1},{2},{3}", new object[] { StartDate, EndDate, empno, Report_Type }).ToList();;
                    viewName = "Others/_Atendencexls";
                    break;

                case "4":

                    //顯示HCP記錄                 
                   Access.HCP = HCPRepository.SqlQuery<HCPFormModel>("sp_Accesswith_Record {0},{1},{2},{3}", new object[] { StartDate, EndDate, empno, Report_Type }).ToList();;
                   viewName = "Others/_HCPxls";
                   break;                

             }

                IView view = ViewEngines.Engines.FindPartialView(ControllerContext, viewName).View;
                var Partia = PartialView(viewName, Access);             
                ViewContext viewContext = new ViewContext(ControllerContext, view, Partia.ViewData, Partia.TempData, writer);
                viewContext.View.Render(viewContext, writer);
                htmlcol.InnerHtml = writer.ToString();
                var file = ToExcel(htmlcol, "~/temp/Others");
